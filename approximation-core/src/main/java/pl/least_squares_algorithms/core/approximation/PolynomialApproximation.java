package pl.least_squares_algorithms.core.approximation;

import Jama.Matrix;
import lombok.extern.slf4j.Slf4j;
import pl.least_squares_algorithms.core.LeastSquaresMethod;
import pl.least_squares_algorithms.core.decomposition.*;
...
public class PolynomialApproximation extends Approximation {
    ...
    public List<MathematicalFunction> doApproximations(LeastSquaresMethod leastSquaresMethod) {
        ...
        log.debug("Decomposition - Matrix A:\n {}", matrixA);

        setMathematicalFunctions(createMathematicalFunction(mapMatrixAToList(matrixA)));
        log.info("Absolute error Polynomial Approximation = {}", calculateError(leastSquaresMethod));
        log.info("Convergence coefficient = {}", calculateConvergenceCoefficient(leastSquaresMethod));
